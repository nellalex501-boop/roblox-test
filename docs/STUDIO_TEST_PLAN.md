...
- [ ] **Canteen from spawn.** Stand on each spawn pad in turn and turn south. → One of the two CANTEEN name boards above the courtyard wall is readable past the Mission Board. From the second pad from the east, the signpost's CANTEEN plate stands in front of the east board, so read that plate instead. Walking up the courtyard, the four planters hold round clipped shrubs, and KANTINE · CANTEEN over the door is in full view.
...
- [ ] **Deployment board in first person.** Just inside the Operations Center door, zoom fully in (first person). → Every line of the deployment board is readable above the QUICK desk's two monitors, down to CURRENT OPERATION and the Zulu clock.
- [ ] **Flush mounting.** Look at signs from the side: the Map Room name plates under the displays, the Hall of Commanders boards, the HQ door plates, the clock labels, and the tower, Map Room and HQ entrance signs. → None stands off its wall with a visible gap. BLDG 203's number is on its north gable, facing the path.
...
- [ ] **Props.** Walk the base. → Benches, lamps, bins, picnic tables and vehicles sit on the ground (none float or sink) and nothing blocks doorways or terminals. Bench backrests lean back on their posts. The awards case in the HQ is an open cabinet with the cups on its shelves. The canteen clock hangs on the back wall over the till, not in the doorway.
- [ ] **Roads and paths.** Walk the loop: the north road west, the west lane behind the Garage, the south road and the east lane behind the Command HQ. → No road ends in the grass, and every barracks door opens onto a footpath that leads to a road.
...
- [ ] **Performance (MicroProfiler, Ctrl+F6).** → Stable 60 FPS on a mid-range PC. On a phone, check with the Developer Console that frame time stays reasonable. There are about 11,800 parts (most of the added ones are decor: no collision, no query, no shadow) and 270 SurfaceGuis. Consider `Workspace.StreamingEnabled` for low-end devices; the client code handles streaming.
